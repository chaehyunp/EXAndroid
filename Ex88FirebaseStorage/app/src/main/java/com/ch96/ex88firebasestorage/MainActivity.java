package com.ch96.ex88firebasestorage;
...
import com.ch96.ex88firebasestorage.databinding.ActivityMainBinding;
...
public class MainActivity extends AppCompatActivity {
    ...
    //Firebase : 안드로이드 앱을 개발할때 서버단 작업을 코딩 한 줄 없이 사용하는 플랫폼
    
    //이 프로젝트와 Firebase 플랫폼을 연동하기! [firebase.com 가이드 문서 참고]
    //사이트에 로그인 후 개발자 console에서 [프로젝트 만들기]를 통해 작업 시작
    ...
    void clickUpload() {
        ...
        //Firebase 저장소 관리 객체 소환
        FirebaseStorage firebaseStorage = FirebaseStorage.getInstance();

        //저장할 파일 위치에 대한 참조객체
        //StorageReference imgRef = firebaseStorage.getReference("test.png"); //test.png가 없으면 만들고 있으면 참조

        //저장할 파일이 중복되지않도록 날짜로 변수명
    ...
    void clickLoad() {
        //Firebase storage에 저장되어있는 이미지 파일을 읽어오기
        ...
        StorageReference imgRef = rootRef.child("zzang_bbaeggom.jpg");

        if (imgRef != null) {
            //파일 참조 객체로부터 이미지의 [다운로드 URL] 얻어오기 - 액세스토큰
